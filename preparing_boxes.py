...
class Slice:
    def slice_box(self, box):
        ...
        if self.x_sliceable(box):
            sliced_intervals.extend(self.x_slice(box))
            box = self.x_cut_off(box)
        if self.y_sliceable(box):
            sliced_intervals.extend(self.y_slice(box))
            box = self.y_cut_off(box)
        if self.z_sliceable(box):
            sliced_intervals.extend(self.z_slice(box))
            box = self.z_cut_off(box)
        #dodanie pudełka wejściowego do listy (po przycięciu)
        sliced_intervals.append(box)
